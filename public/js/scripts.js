document.addEventListener("DOMContentLoaded", () => {
  ...
  g2Form.addEventListener('submit', function (e) {
    ...
    if (licenseNumber.value.length !== 8 || !/^[a-zA-Z0-9]+$/.test(licenseNumber.value)) {
      document.getElementById('licenseError').textContent = 'License number must be exactly 8 alphanumeric characters.';
      isValid = false;
    }

    const age = document.getElementById('age');
    if (age.value < 16 || age.value > 100) {
      document.getElementById('ageError').textContent = 'Age must be between 16 and 100.';
      isValid = false;
    }
    ...
    if (carYear.value < 1900 || carYear.value > 2024) {
